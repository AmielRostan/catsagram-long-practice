export const createCommentSection = () => {
    ...
    const commentsSaved = localStorage.getItem('comments');
    if(commentsSaved) {
        const commentsArray = JSON.parse(commentsSaved);
        commentsArray.forEach(createComment);
    }
};
...
const submitComment = e => {
    ...
    const commentsArrayString = localStorage.getItem('comments');
    if(commentsArrayString) {
        const commentsArray = JSON.parse(commentsArrayString);
        commentsArray.push(commentText);
        localStorage.setItem('comments', JSON.stringify(commentsArray));
    } else {
        const initialComment = [commentText];
        localStorage.setItem('comments', JSON.stringify(initialComment));
    }
}
